...
def clone_repo(git_url: str, branch: str, target_dir: Path, repo_name: str):
    ...
    try:
        ...
        subprocess.run(
            ["git", "clone", "--depth", "1", "--branch", branch, git_url, str(repo_path)],
            check=True,
            capture_output=True,
            text=True
        )
...
def main():
    ...
    parser.add_argument("--json-file", required=True, help="Path to the JSON file containing repository information")
    parser.add_argument("--output-dir", required=True, help="Directory where repositories will be cloned")
